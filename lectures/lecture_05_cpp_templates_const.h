...
Писать реализацию шаблонов можно только в .h файлах.
...
1. vector — это обёртка над массивом (данные расположены линейно в памяти)
...
2. list — это обёртка над списком (данные расположены как угодно в памяти)
...
3. map — это обёртка над ассоциативным массивом
...
Если в ассоциативном массиве обратиться по несущесвующему ключу, то будет краш.

4. string — это обёртка над строками
...
1. Шаблон математического вектора (3д)
	* C перегруженными операциями +, -, * (на константу)
	* Операции получения длины вектора
	* Нормализация
	* dot product
	* cross product
	(+) Везде, где состояние объекта не меняется писать const + const type &
	(+) Переопределить операции +=, -=, *=
	(+) Операторы сравнения ==, !=, должен работать код std::map<класс вектора, не важно что>
